...
struct Route2<T: SceneDependencyType> {

    /// The associated list of scene values.
    let scenes: [SceneInstance<T>]
    ...
    /// Convenience get-only accessor to return the first scene in the route if it exists.
    var firstScene: SceneInstance<T>? { scenes.first }
    ...
    func remainingRoute() -> Route2? {
        ...
        return Route2(scenes: [SceneInstance<T>](scenes.dropFirst()), userIntent: userIntent)
    }
